...
- **[crown-skill](./crown-skill/)** — Make and maintain printed books with [Crown](https://github.com/romellogoodman/crown) (Markdown + Handlebars + CSS → PrinceXML → PDF) and impose the pages onto press sheets with `crown layout`. Carries the conventions the `book-*` projects share — a decoy first page for the Prince stamp, `@page` in `styles.css`, timestamped snapshots, multiple-of-4 page counts, template dispatch by frontmatter — as four reference docs, the shared build script, and a starter quarter-letter zine kit. Assumes crown checked out beside the books at `~/code/crown`.
- **[close-out-skill](./close-out-skill/)** — End a Claude Code session gracefully instead of closing the terminal on it: land and commit the reviewed work (never push), stop what the session started, clean up stray files, save a handoff to memory for the next session, say goodbye, then exit the session itself via a small script that sends the Claude process a clean SIGTERM. Defers to a project's own `close-out` command for build/smoke/install steps when one exists.
